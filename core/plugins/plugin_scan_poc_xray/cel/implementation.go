...
var (
	...
	StandardProgramOption = []cel.ProgramOption{
		cel.Functions(
			&functions.Overload{
...
func NewFunctionImplOptions(reg ref.TypeRegistry) []cel.ProgramOption {
	...
	newOptions = append(newOptions, StandardProgramOption...)
...
func xrayNewReverse() (reverse *structs.Reverse) {
	...
	switch structs.ReversePlatformType {
	case structs.ReverseTypeCeYe:
		sub := utils.RandomStr(utils.AsciiLowercaseAndDigits, 8)
		urlStr = fmt.Sprintf("http://%s.%s/", sub, structs.CeYeDomain)
	case structs.ReverseTypeDnsLogCN:
		dnsLogCnRequest := structs.DnsLogCNGetDomainRequest
	...
	reverse.ReverseType = structs.ReversePlatformType
...
func reverseCheck(r *structs.Reverse, timeout int64) bool {
	...
	case structs.ReverseTypeCeYe:
		...
		urlStr := fmt.Sprintf("http://api.ceye.io/v1/records?token=%s&type=dns&filter=%s", structs.CeYeApi, sub)
	...
	case structs.ReverseTypeDnsLogCN:
		time.Sleep(time.Second * time.Duration(timeout))
		sub := strings.Split(r.Domain, ".")[0]
		resp, _, err := requests.DoRequest(structs.DnsLogCNGetRecordRequest, false)
		if err != nil {
			return false
		}
		content, _ := requests.GetRespBody(resp)
