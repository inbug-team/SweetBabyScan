...
func NewEnvOption() CustomLib {
	...
	c.envOptions = []cel.EnvOption{
		...
		cel.Declarations(
			...
			decls.NewVar("response", decls.NewObjectType("lib.Response")),
			decls.NewVar("reverse", decls.NewObjectType("lib.Reverse")),
		),
		cel.Declarations(
			// functions
...
func (c *CustomLib) UpdateCompileOptions(args map[string]string) {
	for k, v := range args {
		...
		if strings.HasPrefix(v, "randomInt") {
			...
			d = decls.NewVar(k, decls.NewObjectType("lib.Reverse"))
		} else {
...
func DoRequest(req *http.Request, redirect bool) (*models.Response, error) {
	...
	var err error
	if redirect {
		oResp, err = initialize_http_client.HttpClient.Do(req)
	} else {
		oResp, err = initialize_http_client.HttpClientNoRedirect.Do(req)
	}
	if err != nil {
		return nil, err
	}
